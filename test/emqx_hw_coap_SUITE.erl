...
init_per_suite(Config) ->
    ...
    application:set_env(emq_hw_coap, enable_stats, true),
    Config.
...
%send_coap_request(Type, Uri, Method, Payload, Content) ->
%    lwm2m_coap_message:request(Type, Method, Payload, Content),

case01(_Config) ->
    test_mqtt_broker:start_link(),
    {ok, _Started} = application:ensure_all_started(emq_hw_coap),
    UdpSock = open_udp_socket(),
    timer:sleep(100),
    Topic = <<"abc">>, Payload = <<"123">>,
    RegURI = "coap://127.0.0.1/t/r?ep=863703031107431",
    Reply = lwm2m_coap_client:request(post, RegURI),
    {ok,valid, _} = Reply,
    timer:sleep(50),
...
case02(_Config) ->
    ...
    {ok, Pid, N, Code, Content} = lwm2m_coap_observer:observe(Uri),
...
case04(_Config) ->
    ...
    Reply2 = lwm2m_coap_client:request(put, URI2, #coap_content{format = <<"application/octet-stream">>, payload = Payload2}),
    {ok,changed, _} = Reply2,
    timer:sleep(50),
    ...
    Reply3 = lwm2m_coap_client:request(put, URI3, #coap_content{format = <<"application/octet-stream">>, payload = Payload3}),
    {ok,changed, _} = Reply3,
    timer:sleep(50),
...
case06_keepalive(_Config) ->
    ...
    Reply3 = lwm2m_coap_client:request(get, URI3),
...
case07_one_clientid_sub_2_topics(_Config) ->
    ...
    {ok, Pid1, N1, Code1, Content1} = lwm2m_coap_observer:observe(Uri1),
    ...
    Payload2 = <<"456">>,
    Uri2 = "coap://127.0.0.1/mqtt/"++TopicStr2++"?c=client1&u=tom&p=secret",
    {ok, Pid2, N2, Code2, Content2} = lwm2m_coap_observer:observe(Uri2),
    ...
    lwm2m_coap_observer:stop(Pid1),
    lwm2m_coap_observer:stop(Pid2),
    timer:sleep(100),
    SubTopicFinal = test_mqtt_broker:get_subscrbied_topics(),
    ?_assertEqual([], SubTopicFinal),
...
case11_invalid_parameter(_Config) ->
    ...
    Reply4 = lwm2m_coap_client:request(put, URI4, #coap_content{format = <<"application/octet-stream">>, payload = Payload3}),
...
case13_emit_stats_test(_Config) ->
    ...
    Reply = lwm2m_coap_client:request(put, URI, #coap_content{format = <<"application/octet-stream">>, payload = Payload}),
